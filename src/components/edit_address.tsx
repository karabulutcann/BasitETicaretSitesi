...
import { Building2, Check, House, MapPin, PenLine, Plus } from "lucide-react";
...
export default function EditAddress({
  ...
  callbackFn:
    | undefined
    | ((address: typeof addressSchema.$inferSelect) => void);
}) {
  const [city, setCity] = React.useState<string>(address.city);
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="ghost" className={buttonClassName}>
          düzenle <PenLine className="h-4 w-4 ml-2" />
        </Button>
      </DialogTrigger>
        ...
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit((data) => {
              setIsLoading(true);
              fetch(baseUrl + "/api/address/edit_address", {
                method: "POST",
            ...
            className=" gap-8 grid grid-cols-2 py-2"
          >
            <FormField
