import { TH1, TH2, TH3, TH4 } from "@/components/typography";
...
export default async function Page({ params }: { params: { id: string } }) {
  try {
    ...
    const order = await db.query.orderSchema.findFirst({
      where: and(
        eq(orderSchema.id, params.id),
        eq(orderSchema.userId, session!.user?.id!),
      ),
      with: {
        address: true,
        orderDetail: {
              ...
              <CardContent className="py-4 flex grow flex-col justify-between">
                ...
                <div className="px-2 flex ">
                  ...
                  >
                    ...
                    <div className="text-sm">
                      tarih: {dateFormat.format(order?.hasBeenShippedAt!)}
                    </div>
                  </div>
                </div>
                <div className="px-2 flex ">
                  ...
                  >
                    ...
                    <div className="text-sm">
                      tarih: {dateFormat.format(order?.onTheWayAt!)}
                    </div>
                  </div>
                </div>
                <div className="px-2 flex ">
                  ...
                  >
                    ...
                    <div className="text-sm">
                      tarih: {dateFormat.format(order?.deliveredAt!)}
                    </div>
                  </div>
                </div>
              </CardContent>
